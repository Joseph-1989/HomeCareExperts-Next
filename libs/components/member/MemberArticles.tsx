...
const MemberArticles: NextPage = ({ initialInput, ...props }: any) => {
	...
	const {
		...
		onCompleted: (data: any) => {
			setMemberBoArticles(data?.getBoardArticles?.list); // Optional chaining for safety
			setTotal(data?.getBoardArticles?.metaCounter?.[0]?.total || 0); // Optional chaining and array access
		},
	});
	...
	const likeArticleHandler = async (e: any, user: any, id: string) => {
		try {
			...
			console.log('ERROR, likeArticleHandler:', err.message); // Use the correct handler name
