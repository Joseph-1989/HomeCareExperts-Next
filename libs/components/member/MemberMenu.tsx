...
const MemberMenu = (props: MemberMenuProps) => {
	...
	const {
		...
		skip: !memberId, // Skip the query if memberId is not provided
		notifyOnNetworkStatusChange: true, // Update the UI on network changes
		onCompleted: (data: T) => {
			// Correct syntax for onCompleted
			setMember(data?.getMember); // Use optional chaining to set member data
		},
	});
					...
					<Stack className={'section'}>
						...
						<List className={'sub-section'}>
							{member?.memberType === 'AGENT' && (
								<ListItem className={category === 'services' ? 'focus' : ''}>
									<Link
										href={{
											pathname: '/member',
