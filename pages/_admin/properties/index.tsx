...
const AdminProperties: NextPage = ({ initialInquiry, ...props }: any) => {
	...
	const changePageHandler = async (event: unknown, newPage: number) => {
		propertiesInquiry.page = newPage + 1; // Update page in the inquiry object

		await getAllPropertiesByAdminRefetch({ input: propertiesInquiry });
		setPropertiesInquiry({ ...propertiesInquiry }); // Update state after refetch
	};

	const changeRowsPerPageHandler = async (event: React.ChangeEvent<HTMLInputElement>) => {
		const newLimit = parseInt(event.target.value, 10);
		propertiesInquiry.limit = newLimit; // Update limit in the inquiry object
		propertiesInquiry.page = 1; // Reset page to 1 when limit changes
	...
	const updatePropertyHandler = async (updateData: PropertyUpdate) => {
		try {
			...
			menuIconCloseHandler(); // Move menuIconCloseHandler to the try block after updatePropertyByAdmin has finished
			await getAllPropertiesByAdminRefetch({ input: propertiesInquiry });
		} catch (err: any) {
