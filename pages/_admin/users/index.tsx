...
const AdminUsers: NextPage = ({ initialInquiry, ...props }: any) => {
	...
	const {
		...
		onCompleted: (data: T) => {
			setMembers(data?.getAllMembersByAdmin?.list); // Optional chaining for safety
			setMembersTotal(data?.getAllMembersByAdmin?.metaCounter?.[0]?.total ?? 0); // Optional chaining & nullish coalescing
		},
	});
	...
	const updateMemberHandler = async (updateData: MemberUpdate) => {
		try {
			...
			menuIconCloseHandler(); // Close a menu or modal (likely)

			await getAllMembersRefetch({ input: membersInquiry });
		} catch (err: any) {
			sweetErrorHandling(err).then(); // Handle errors
		}
	};
